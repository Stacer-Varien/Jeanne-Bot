...
class imagehelp(ui.Select):
    ...
    async def callback(self, ctx: Interaction):
        ...
        if self.values[0] == "Wallpaper":
            ...
            await ctx.followup.send(embed=wallpaper, ephemeral=True)
        if self.values[0] == "Jeanne":
            ...
            await ctx.followup.send(embed=jeanne, ephemeral=True)
        ...
        if self.values[0] == "Neko":
            ...
            await ctx.followup.send(embed=neko, ephemeral=True)
