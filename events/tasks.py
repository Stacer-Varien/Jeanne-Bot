...
class tasksCog(Cog):
    ...
    async def check_softbanned_members(self):
        for bans in Moderation().get_softban_data() or []:
            if int(round(datetime.now().timestamp())) <= int(bans[2]):
                continue

            try:
                guild = self.bot.get_guild(int(bans[1])) or await self.bot.fetch_guild(
                    int(bans[1])
                )
                member = await self.bot.fetch_user(int(bans[0]))
            except Exception:
                continue
            ...
            await Moderation(guild).remove_softban(member)
            ...
            if locale == "en":
                ...
                unmute.add_field(name="ID", value=member.id, inline=True)
                unmute.add_field(name="Raison", value="Softban expiré", inline=True)
                unmute.set_thumbnail(url=member.display_avatar)
            else:
                unmute = Embed(title="Mitglied entbannt", color=0xFF0000)
                unmute.add_field(name="Mitglied", value=member, inline=True)
                unmute.add_field(name="ID", value=member.id, inline=True)
                unmute.add_field(name="Grund", value="Softban abgelaufen", inline=True)
                unmute.set_thumbnail(url=member.display_avatar)
