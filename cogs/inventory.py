        ...
        if Botban(ctx.user).check_botbanned_user() == True:
            ...
            try:
                ...
                embed = Embed(description=
                              "This background image is not in your inventory",
                              color=Color.red())
                await ctx.followup.send(embed=embed)
    ...
    async def buycustom(self, ctx: Interaction, name: str, link: str):
        ...
        if Botban(ctx.user).check_botbanned_user() == True:
            ...
            if balance == None:
                ...
                qp = self.bot.get_emoji(980772736861343774)
                await ctx.followup.send(
                    "Creating preview... This will take some time {}".format(
                        loading))
                ...
                preview = Embed(
                    description="This is the preview of the level card.",
                    color=Color.blue()
                ).add_field(name="Cost", value=f"1000 {qp}").set_footer(
                    text="Is this the background you wanted?"
                ).set_footer(
                    text=
                    "Please note that if the custom background violates ToS or is NSFW, it will be removed with NO REFUNDS!"
                )
                view = Confirmation(ctx.user)
                await ctx.edit_original_response(content=None,
                                                 embed=preview,
