from discord import ButtonStyle, Color, Embed, TextStyle, app_commands as Jeanne, ui
...
class ReportModal(ui.Modal, title="Bot Report"):
    def __init__(self, report_type:str):
        ...
        if self.report_type == "Fault" or "Bug":
            report=ui.TextInput(label="What problem have you found?", placeholder="Type the problem here", required=True, min_length=10, max_length=2000, style=TextStyle.paragraph)

            self.add_item(report)

        elif self.report_type== "Violator":
            violator = ui.TextInput(label="What is the user ID of the person violating the bot's ToS?",
                                    placeholder="Add user ID here",
                                    required=True,
                                    min_length=10,
                                    max_length=15,
                                    style=TextStyle.short)
            report = ui.TextInput(label="What has this person done?",
                                  placeholder="Type the problem here",
                                  required=True,
                                  min_length=10,
                                  max_length=2000,
                                  style=TextStyle.paragraph)
            self.add_item(violator)
            self.add_item(report)
...
class Weather_Group(GroupCog, name="weather"):
    ...
    @Jeanne.command(description="Get weather information on a city but with a ZIP code and Country code")
...
class Embed_Group(GroupCog, name="embed"):
    ...
    @Jeanne.command(description="Generates an embed message. This needs the Discohooks.org embed generator")
    @Jeanne.describe(channel="Send to which channel?", jsonscript="Add a JSON script", jsonfile="Add a JSON file")
    ...
    @Jeanne.command(description="Edits an embed message. This needs the Discohook.org embed generator")
    @Jeanne.describe(channel="Which channel is the embed message in?", messageid="What is the message ID?", jsonscript="Add a JSON script", jsonfile="Add a JSON file")
    @Jeanne.checks.has_permissions(administrator=True)
...
class slashutilities(Cog):
    ...
    @Jeanne.describe(type="What is the nature of the report?")
    ...
    @Jeanne.command(description="Report a member in your server")
    @Jeanne.describe(member="Who are you reporting?", anonymous="Do you want your name to be hidden?")
