        ...
        if Botban(ctx.user).check_botbanned_user() == True:
            ...
            if image.endswith('mp4'):
                ...
                embed = Embed(color=Color.purple()).set_image(
                        url=image).set_footer(text="Fetched from Gelbooru • Credits must go to the artist")
                await ctx.followup.send(embed=embed)
    ...
    async def yandere(self, ctx: Interaction, rating: Optional[Literal["questionable", "explicit"]], tag: Optional[str] = None) -> None:
        ...
        if Botban(ctx.user).check_botbanned_user() == True:
            pass
                ...
                yandere.set_footer(text="Fetched from Yande.re • Credits must go to the artist")
    ...
    async def konachan(self, ctx: Interaction, rating: Optional[Literal["questionable", "explicit"]], tag: Optional[str] = None) -> None:
        ...
        if Botban(ctx.user).check_botbanned_user() == True:
            pass
                ...
                konachan.set_footer(text="Fetched from Konachan • Credits must go to the artist")
