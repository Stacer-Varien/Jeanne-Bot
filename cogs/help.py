...
class help(commands.Cog):
    ...
    async def Fun(self, ctx):
            ...
            await Paginator(bot=self.bot, ctx=ctx, pages=pages).run()
    ...
    async def mod(self, ctx):
            ...
            purge = discord.Embed(
                title="Purge help",
                description="Bulk delete messages.\n\n**Required Permission:** Manage Messages\n\n**NOTE:** Will delete up to 100 messages. You can also mention a member or add a number less than 50 to delete the messages",
                color=0x093cb3)
            purge.add_field(
                name="Example:", value="j!purge 20 MEMBER\n**NOTE**: For purging messages up to the limit, use `j!purge 100`", inline=False)

            mute = Embed(
                title="Mute help",
                description="Mute someone and they will not talk.\n\n**Required Permission:** Kick Members\n\n**NOTE:** If a mute role doesn't exist, a new one will made. If there is no time given, it will be infinite. All times displayed by Jeanne will be in seconds instead of actual time duration",
                color=0x093cb3)
            mute.add_field(
