...
class slashinfo(Cog):
    ...
    async def userinfo(self,
        ...
        if Botban(ctx.user).check_botbanned_user() == True:
            ...
            userinfo.add_field(name="Created Account",
                               value='<t:{}:F>'.format(str(create_date)),
                               inline=True)
            userinfo.add_field(name="Joined Server",
    ...
    async def serverinfo(self, ctx: Interaction):
        ...
        if Botban(ctx.user).check_botbanned_user() == True:
            ...
            if len(emojis) == 0:
                ...
                emojie = Embed(title="Emojis",
                               description=''.join(emojis[:40]),
                               color=Color.random())
    ...
    async def ping(self, ctx: Interaction):
        if Botban(ctx.user).check_botbanned_user() == True:
            ...
            ping.add_field(name="Bot Latency",
                           value=f'{round(self.bot.latency * 1000)}ms',
                           inline=False)
            end_time = time()
            ping.add_field(name="API Latency",
    ...
    async def serveravatar(self,
        ...
        if Botban(ctx.user).check_botbanned_user() == True:
            ...
            guild_avatar = Embed(title=f"{member}'s Avatar",
                                 color=member.color)
            ...
            if member_avatar == True:
                ...
                guild_avatar.set_footer(
                    text=
                    "Member has no server avatar. Passed normal avatar instead"
                )
                await ctx.followup.send(embed=guild_avatar)
    ...
    async def sticker_error(self, ctx: Interaction,
        ...
        if isinstance(error, Jeanne.CommandInvokeError):
            if IndexError:
                embed = Embed(
                    description=
                    "No sticker is in that message or the sticker got deleted from its server",
                    color=Color.red())
                await ctx.followup.send(embed=embed)
            elif AttributeError:
                embed = Embed(
