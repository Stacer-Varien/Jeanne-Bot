from discord import Color, Embed


DOCS_URL = "https://jeannebot.vercel.app/help"
SUPPORT_URL = "https://discord.gg/jh7jkuk2pp"

TEXT = {
    "setup_title": "Server Setup",
    "setup_description": "Private setup panel for this server.",
    "status_channels": "Configured Channels",
    "status_language": "Language",
    "status_modules": "Module Toggles",
    "status_currency": "Currency Display",
    "not_set": "Not set",
    "missing_channel": "Missing channel",
    "auto_language": "Auto (server locale)",
    "disabled_none": "No disabled modules",
    "area_placeholder": "Choose what channel to configure",
    "channel_placeholder": "Choose the channel to use",
    "language_placeholder": "Choose a server language",
    "module_placeholder": "Toggle a module",
    "currency_button": "Currency",
    "refresh_button": "Refresh",
    "unauthorized": "Only the person who opened setup can use this panel.",
    "select_area_first": "Choose a setup area first, then choose the channel.",
    "updated": "Setting updated.",
    "currency_modal_title": "Currency Display",
    "currency_name_label": "Currency name",
    "currency_emoji_label": "Currency emoji",
    "diagnose_title": "Server Diagnostics",
    "diagnose_description": "Operational checks for Jeanne in this server.",
    "bot_permissions": "Bot Permissions",
    "member_permissions": "Your Permissions",
    "channel_health": "Channel Health",
    "role_hierarchy": "Role Hierarchy",
    "configured_channels": "Configured Channel Health",
    "settings": "Server Settings",
    "intents": "Gateway Intents",
    "case_not_found": "I could not find that moderation case.",
    "case_no_cases": "No moderation cases found.",
    "case_view_title": "Moderation Case",
    "case_recent_title": "Recent Moderation Cases",
    "case_user_title": "User Moderation Cases",
    "target": "Target",
    "moderator": "Moderator",
    "reason": "Reason",
    "duration": "Duration",
    "status": "Status",
    "warn_id": "Warn ID",
    "date": "Date",
}

AREA_LABELS = {
    "welcome": "Welcome",
    "leave": "Leave",
    "modlog": "Modlog",
    "level": "Level updates",
    "confession": "Confessions",
}

LANGUAGE_LABELS = {
    "auto": "Auto",
    "en": "English",
    "fr": "French",
    "de": "German",
}


def channel_status(state: dict) -> str:
    if state["channel"]:
        return state["channel"].mention
    if state["missing"]:
        return f"{TEXT['missing_channel']} `{state['channel_id']}`"
    return TEXT["not_set"]


def setup_embed(guild, settings, status: dict) -> Embed:
    embed = Embed(
        title=TEXT["setup_title"],
        description=TEXT["setup_description"],
        color=Color.blurple(),
    )
    embed.set_author(name=guild.name, icon_url=guild.icon.url if guild.icon else None)
    embed.add_field(
        name=TEXT["status_channels"],
        value="\n".join(
            f"**{AREA_LABELS[key]}:** {channel_status(status[key])}"
            for key in ("welcome", "leave", "modlog", "level", "confession")
        ),
        inline=False,
    )
    language = status["language_override"]
    embed.add_field(
        name=TEXT["status_language"],
        value=TEXT["auto_language"] if language == "auto" else LANGUAGE_LABELS[language],
        inline=True,
    )
    disabled = status["disabled_modules"]
    embed.add_field(
        name=TEXT["status_modules"],
        value=(
            TEXT["disabled_none"]
            if not disabled
            else ", ".join(settings.MODULES[module] for module in disabled)
        ),
        inline=True,
    )
    currency_value = settings.format_currency(1).replace("1 ", "", 1)
    embed.add_field(name=TEXT["status_currency"], value=currency_value, inline=True)
    return embed


def diagnose_embed(guild, report: dict) -> Embed:
    embed = Embed(
        title=TEXT["diagnose_title"],
        description=TEXT["diagnose_description"],
        color=Color.blurple(),
    )
    embed.set_author(name=guild.name, icon_url=guild.icon.url if guild.icon else None)
    for key in (
        "bot_permissions",
        "member_permissions",
        "channel_health",
        "role_hierarchy",
        "configured_channels",
        "settings",
        "intents",
    ):
        embed.add_field(
            name=TEXT[key],
            value="\n".join(report[key])[:1024] or "OK",
            inline=False,
        )
    return embed


def case_embed(case: tuple, target: str, moderator: str) -> Embed:
    embed = Embed(title=f"{TEXT['case_view_title']} #{case[1]}", color=Color.red())
    embed.add_field(name="Action", value=str(case[2]).title(), inline=True)
    embed.add_field(name=TEXT["target"], value=target, inline=True)
    embed.add_field(name=TEXT["moderator"], value=moderator, inline=True)
    embed.add_field(name=TEXT["reason"], value=case[5] or "Unspecified", inline=False)
    embed.add_field(name=TEXT["date"], value=f"<t:{case[6]}:F>", inline=True)
    embed.add_field(name=TEXT["status"], value=case[8] or "active", inline=True)
    if case[7]:
        embed.add_field(name=TEXT["duration"], value=case[7], inline=True)
    if case[9] is not None:
        embed.add_field(name=TEXT["warn_id"], value=str(case[9]), inline=True)
    return embed


def case_list_embed(title: str, lines: list[str]) -> Embed:
    if not lines:
        return Embed(description=TEXT["case_no_cases"], color=Color.red())
    return Embed(title=title, description="\n".join(lines)[:4096], color=Color.red())
